...
class RemoteControlClientConfig implements RemoteControlConfig {
  ...
  RemoteControlClientConfig(
      {this.port = remoteControlDefaultPort,
      this.clientType =
          RemoteControlConnectionTypeEnum.remoteControlledClient});
}
...
class RemoteControlMixedConfigForTesting implements RemoteControlConfig {
  ...
  RemoteControlMixedConfigForTesting(
      {this.port = remoteControlDefaultTestPort});
}
...
class RemoteControlRegisterTextCommandConfig
    implements RemoteControlRegisterCommandConfig {
  @override
  String name;
...
class ConnectionSecurityDelegate {
...
class ClientsWorkspace implements Workspace {
  final List<ConnectionSecurityDelegate> connectionSecurityDelegate;
  final List<GroupDelegate> groupDelegate;
  final List<Workspace> workspaces;

  ClientsWorkspace(this.connectionSecurityDelegate, this.groupDelegate, this.workspaces);
}

mixin GroupWorkspaceMixin implements Workspace {
  String get name;

}

class RunningProductionAppGroupWorkspace with GroupWorkspaceMixin {
  @override
  String get name => 'Running production apps';
}